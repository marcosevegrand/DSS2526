...
Um dos resultados mais significativos foi a separa\c{c}\~ao do sistema em tr\^es subsistemas distintos, mas complementares, cada um com a sua pr\'opria fachada especializada. O subsistema de venda (\textit{VendaFacade}) gere a intera\c{c}\~ao direta com o cliente, permitindo a visualiza\c{c}\~ao do cat\'alogo e a personaliza\c{c}\~ao de produtos. O subsistema de produ\c{c}\~ao (\textit{ProducaoFacade}) organiza as tarefas nas esta\c{c}\~oes de trabalho, calculando tempos de espera reais. Por fim, o subsistema de gest\~ao (\textit{GestaoFacade}) oferece as ferramentas administrativas para o controlo de pessoal, stocks e configura\c{c}\~ao do sistema.
...
Estes diagramas ilustram as intera\c{c}\~oes entre os atores (Cliente, Cozinheiro, Gestor) e o sistema.
...
\begin{figure}[H]
    \centering
    \framebox{\parbox{0.8\textwidth}{\centering \vspace{2.5cm} \textbf{Diagrama de Classe} \\ \small (Placeholder) \vspace{2.5cm}}}
    \quad
    \framebox{\parbox{0.8\textwidth}{\centering \vspace{2.5cm} \textbf{Diagrama de Sequ\^encia} \\ \small (Placeholder) \vspace{2.5cm}}}
    \caption{Estrutura da solu\c{c}\~ao implementada.}
\end{figure}
...
\begin{figure}[H]
    ...
    \hfill
    \framebox{\parbox{0.45\textwidth}{\centering \vspace{2cm} \textbf{Diagrama de Sequ\^encia} \\ \small (Placeholder) \vspace{2cm}}}
    \caption{Detalhamento da solu\c{c}\~ao implementada.}
\end{figure}
...
O cliente interage com o terminal de venda para gerir os seus pedidos. As principais funcionalidades s\~ao:

\begin{itemize}
    \item Visualizar os pedidos ativos do restaurante
    \item Iniciar um novo pedido
    \item Consultar os itens dispon\'iveis no cat\'alogo
    \item Adicionar itens ao pedido
    \item Remover itens do pedido
    \item Finalizar o pedido e escolher o local de pagamento
    \item Receber um recibo com o n\'umero do pedido e o tempo estimado de entrega
    \item Consultar as estat\'isticas do restaurante
\end{itemize}

A \textit{VendaFacade} coordena todas estas opera\c{c}\~oes, gerenciando o fluxo de cria\c{c}\~ao e manipula\c{c}\~ao de pedidos at\'e \`a finaliza\c{c}\~ao.

\subsection{Subsistema de Produ\c{c}\~ao (Cozinha)}
O funcion\'ario na cozinha utiliza este subsistema para gerir a produ\c{c}\~ao dos pedidos. O funcion\'ario escolhe o restaurante e a sua esta\c{c}\~ao espec\'ifica. O sistema apresenta as tarefas a realizar. Ao concluir um item, o estado \'e atualizado em tempo real, permitindo que o cliente seja informado quando o pedido est\'a pronto para entrega.
...
O gestor utiliza este subsistema para gerenciar os aspetos operacionais e administrativos do restaurante. Pode monitorizar o invent\'ario de ingredientes, gerir os funcion\'arios e suas permiss\~oes, configurar cat\'alogos e menus, e acompanhar o desempenho do restaurante.

A \textit{GestaoFacade} oferece:
\begin{itemize}
    \item Controlo de stocks e ingredientes
    \item Gest\~ao de funcion\'arios e defini\c{c}\~ao de permiss\~oes
    \item Configura\c{c}\~ao de cat\'alogos e menus
    \item Monitoriza\c{c}\~ao de pedidos e relatorios operacionais
\end{itemize}
