...
public class VendaController {
    ...
    // Caches para garantir correspondência de índices UI -> Dados
    ...
    public void selecionarRestaurante(int idx) { 
        // Validação de índice
        if (idx >= 0 && idx < cacheRests.size()) {
            this.rId = cacheRests.get(idx).getId(); 
        }
    }
    ...
    public void iniciarNovoPedido() { 
        ...
        this.cacheItems.clear(); // Limpar cache antiga
    }
    ...
    public void removerItem(int idx, int qtd) {
        ...
        // Validação Robusta de Índice
        // O índice visual do carrinho corresponde à ordem da lista de linhas
        if (idx >= 0 && idx < linhas.size()) {
            facade.removerQuantidadeDoPedido(pId, linhas.get(idx).getId(), qtd);
        }
    }
    ...
    public String pagar(int opt) { 
        ...
        if (tp == TipoPagamento.TERMINAL && est != null) {
            return "PEDIDO CONFIRMADO. Tempo estimado: " + est.toMinutes() + " minutos.";
        } else {
            return "PEDIDO ENVIADO. Por favor, dirija-se à caixa para finalizar o pagamento.";
        }
    }
    
    public void cancelar() { 
        facade.cancelarPedido(pId); 
    }
