...
public class ProducaoController {
    ...
    public List<String> getRestaurantes() { 
        ...
        return cacheRestaurantes.stream().map(Restaurante::getNome).collect(Collectors.toList()); 
    }
    ...
    public List<String> getEstacoes() { 
        ...
        return cacheEstacoes.stream().map(Estacao::getNome).collect(Collectors.toList()); 
    }
    ...
    public void iniciarTarefaPendente(int uiIndex) { 
        facade.iniciarTarefa(cacheTaskIdsPendentes.get(uiIndex), this.eId); 
    }
    ...
    public List<String> getTarefasEmExecucao() {
        ...
        return ts.stream()
                 .map(t -> "Pedido #" + t.getPedidoId() + " : " + facade.obterPasso(t.getPassoId()).getNome() + " [" + t.getEstado() + "]")
                 .collect(Collectors.toList());
    }

    public void concluirTarefaEmExecucao(int uiIndex) {
        facade.concluirTarefa(cacheTaskIdsEmExecucao.get(uiIndex));
    }
    ...
    public void atrasarTarefaEmExecucao(int taskUiIndex, int ingUiIndex) { 
        facade.atrasarTarefa(cacheTaskIdsEmExecucao.get(taskUiIndex), cacheIngIds.get(ingUiIndex)); 
    }
    ...
    // Agora devolve String com a estimativa
    public String confirmarPagamento(int i) { 
        Duration estimativa = facade.processarPagamentoCaixa(cacheOrderIds.get(i)); 
        return "Pagamento confirmado. Tempo estimado de espera: " + estimativa.toMinutes() + " minutos.";
    }
    ...
    public List<String> getLinhasPedido(int i) { 
        return facade.obterPedido(cacheOrderIds.get(i)).getLinhas().stream()
            .map(l -> {
                String item = (l.getTipo() == TipoItem.PRODUTO) 
                    ? facade.obterProduto(l.getItemId()).getNome() 
                    : facade.obterMenu(l.getItemId()).getNome();
                return item + " (Qtd: " + l.getQuantidade() + ")";
            })
            .collect(Collectors.toList()); 
    }
    ...
    public void solicitarRefacao(int pIdx, List<Integer> lIdxs) {
        ...
        List<Integer> lines = lIdxs.stream()
            .filter(idx -> idx >= 0 && idx < p.getLinhas().size())
            .map(idx -> p.getLinhas().get(idx).getId())
            .collect(Collectors.toList());
    ...
    public List<String> getMonitorGlobal() { 
        return facade.obterProgressoMonitor(rId).entrySet().stream()
            ...
            .map(e -> String.format("Pedido #%d [%-12s] -> %s", 
                    e.getKey().getId(), e.getKey().getEstado(), e.getValue()))
            .collect(Collectors.toList()); 
    }

    public List<String> getMensagens() {
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm");
        return facade.listarMensagens(rId).stream()
            .map(m -> "[" + m.getDataHora().format(dtf) + "] " + m.getTexto())
            .collect(Collectors.toList());
    }
}