...
public class MenuDAOImpl implements MenuDAO {
    ...
    private final Map<Integer, LinhaMenu> linhaMenuMap = new HashMap<>();

    private MenuDAOImpl() { this.dbConfig = DBConfig.getInstance(); }
    ...
    public Menu create(Menu entity) {
        ...
        try (Connection conn = dbConfig.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (ResultSet rs = ps.getGeneratedKeys()) { if (rs.next()) { entity.setId(rs.getInt(1)); menuMap.put(entity.getId(), entity); } }
    ...
    private void insertLinhas(Connection conn, Menu entity) throws SQLException {
        ...
        try (PreparedStatement psL = conn.prepareStatement(sqlLinha, Statement.RETURN_GENERATED_KEYS)) {
            for (LinhaMenu lm : entity.getLinhas()) {
                psL.setInt(1, entity.getId()); psL.setInt(2, lm.getProdutoId()); psL.setInt(3, lm.getQuantidade()); psL.executeUpdate();
                try (ResultSet rs = psL.getGeneratedKeys()) { if (rs.next()) { lm.setId(rs.getInt(1)); lm.setMenuId(entity.getId()); linhaMenuMap.put(lm.getId(), lm); } }
            }
        }
    }
    ...
    public Menu update(Menu entity) {
        ...
        try (Connection conn = dbConfig.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (PreparedStatement psDel = conn.prepareStatement("DELETE FROM LinhaMenu WHERE menu_id=?")) { psDel.setInt(1, entity.getId()); psDel.executeUpdate(); }
                insertLinhas(conn, entity);
                conn.commit(); menuMap.put(entity.getId(), entity);
            } catch (SQLException e) { conn.rollback(); throw e; }
        } catch (SQLException e) { e.printStackTrace(); }
        return entity;
    }
    ...
    public Menu findById(Integer id) {
        if (menuMap.containsKey(id)) return menuMap.get(id);
        String sql = "SELECT * FROM Menu WHERE id = ?";
        try (Connection conn = dbConfig.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    Menu m = new Menu(); m.setId(rs.getInt("id")); m.setNome(rs.getString("nome")); m.setPreco(rs.getDouble("preco"));
    ...
    private List<LinhaMenu> findLinhas(Connection conn, int menuId) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    int id = rs.getInt("id");
                    if (linhaMenuMap.containsKey(id)) list.add(linhaMenuMap.get(id));
                    else {
                        LinhaMenu lm = new LinhaMenu(); lm.setId(id); lm.setMenuId(rs.getInt("menu_id")); lm.setProdutoId(rs.getInt("produto_id")); lm.setQuantidade(rs.getInt("quantidade"));
                        linhaMenuMap.put(id, lm); list.add(lm);
                    }
                }
            }
        }
    ...
    @Override public boolean delete(Integer id) {
        ...
        try (Connection conn = dbConfig.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id); int rows = ps.executeUpdate(); if (rows > 0) menuMap.remove(id); return rows > 0;
