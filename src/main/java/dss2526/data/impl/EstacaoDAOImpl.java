...
/**
 * Implementação do EstacaoDAO focada na persistência de estações de Cozinha e Caixa.
 * O conceito de Balcão Híbrido foi removido desta implementação.
 */
public class EstacaoDAOImpl implements EstacaoDAO {
        ...
        try (Connection conn = dbConfig.getConnection();
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    ...
                    saveTrabalhos(conn, entity); // Guarda as especialidades de trabalho
    ...
    public Estacao update(Estacao entity) {
        ...
        try (Connection conn = dbConfig.getConnection()) {
            ...
            saveTrabalhos(conn, entity); // Atualiza os trabalhos associados
            estacaoMap.put(entity.getId(), entity);
        } catch (SQLException e) { e.printStackTrace(); }
        return entity;
    }
    ...
    private void saveTrabalhos(Connection conn, Estacao e) throws SQLException {
        // Apenas Cozinhas têm especialidades de trabalho nesta lógica
        if (e instanceof Estacao.Cozinha cozinha) {
            ...
            // Inserir as especialidades atuais
    ...
    private Estacao map(ResultSet rs, Connection conn) throws SQLException {
        ...
        // Mapeamento simplificado sem Balcão Híbrido
        Estacao e = switch(tipo) {
            case "CAIXA" -> new Estacao.Caixa();
            default -> new Estacao.Cozinha();
        };
        ...
        // Se for Cozinha, carregar as especialidades de trabalho
        if (e instanceof Estacao.Cozinha cozinha) {
            try (PreparedStatement ps = conn.prepareStatement("SELECT trabalho FROM Estacao_Trabalho WHERE estacao_id = ?")) {
                ps.setInt(1, e.getId());
                try (ResultSet rsT = ps.executeQuery()) {
                    while (rsT.next()) {
                        cozinha.addEspecialidade(Trabalho.valueOf(rsT.getString("trabalho")));
                    }
                }
            }
        }
    ...
    private String getDiscriminator(Estacao e) {
        if (e instanceof Estacao.Caixa) return "CAIXA";
        return "COZINHA";
    }
